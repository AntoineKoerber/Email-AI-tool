...
function transformEmailRecord(record: any) {
  return {
    ...
    fields: {
      ...
      pref_comm: record.pref_comm, // Add this line
    },
...
export async function GET() {
  try {
    ...
    if (IS_DEMO_MODE) {
      const transformedRecords = mockEmails.map(transformEmailRecord)
  ...
  try {
    ...
    const supabaseRecords = body.records.map((record: any) => {
      ...
      return {
        ...
        pref_comm: record.fields.pref_comm || "unknown", // Add pref_comm field
      }
    })
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (body.records && body.records.length > 0) {
      ...
      // Prepare update data - handle all editable fields
      const updateData: any = {}

      // Status field
      if (record.status) {
        updateData.status = record.status
        console.log("Setting status to:", record.status)
      }

      // Reply status field
      if (record.reply_status) {
        updateData.reply_status = record.reply_status
        console.log("Setting reply_status to:", record.reply_status)
      }

      // Operator field
      if (record.operator) {
        updateData.operator = record.operator
        console.log("Setting operator to:", record.operator)
      }

      // Incentive field
      if (record.incentive) {
        updateData.incentive = record.incentive
        console.log("Setting incentive to:", record.incentive)
      }

      // Follow-up fields
      if (typeof record.follow_up_1 === "boolean") {
        updateData.follow_up_1 = record.follow_up_1
        console.log("Setting follow_up_1 to:", record.follow_up_1)
      }

      if (typeof record.follow_up_2 === "boolean") {
        updateData.follow_up_2 = record.follow_up_2
        console.log("Setting follow_up_2 to:", record.follow_up_2)
      }

      // Communication preference field
      if (record.pref_comm) {
        updateData.pref_comm = record.pref_comm
        console.log("Setting pref_comm to:", record.pref_comm)
      }

      // Final email fields (for backward compatibility with review functionality)
